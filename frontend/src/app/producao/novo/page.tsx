...
export default function NovaProducao() {
  ...
  const adicionarHoras = (horas: number) => {
    ...
    const isoLocal = new Date(dataInicio.getTime() - offset)
      .toISOString()
      .slice(0, 16);
  ...
  async function buscarTemperaturas() {
    ...
    try {
      ...
      if (!token) {
        setToast({ msg: "Sessão expirada. Faça login.", type: "error" });
        return;
      }
      ...
      const res = await fetch(
        `https://padaria-royal-api.onrender.com/producao/clima-previsao?${query}`,
        {
          headers: {
      ...
      if (data && data.tempInicial != null && data.tempFinal != null) {
        setFormData((prev) => ({
          ...prev,
          tempAmbienteInicial: String(data.tempInicial),
          tempAmbienteFinal: String(data.tempFinal),
        }));
      } else {
        setToast({
          msg: data.aviso || "Sem previsão disponível.",
          type: "error",
        });
      }
    } catch (error) {
  ...
  async function pedirSugestao() {
    ...
    if (
      !formData.tempAmbienteInicial ||
      Number(formData.tempAmbienteInicial) <= 0 ||
      !formData.tempAmbienteFinal ||
      Number(formData.tempAmbienteFinal) <= 0
    ) {
      setToast({
        msg: "Insira a temperatura",
        type: "error",
      });
      return;
    }
    ...
    if (!farinha || farinha <= 0) {
      setToast({
        msg: "Digite a quantidade de farinha primeiro.",
        type: "error",
      });
      return;
    }
    ...
    try {
      ...
      let url = `https://padaria-royal-api.onrender.com/producao/sugestao?farinha=${farinha}`;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const payload = {
        ...
        tempAmbienteInicial: formData.tempAmbienteInicial
          ? safeParse(formData.tempAmbienteInicial)
          : null,
        tempAmbienteFinal: formData.tempAmbienteFinal
          ? safeParse(formData.tempAmbienteFinal)
          : null,
      };

      const res = await fetch("https://padaria-royal-api.onrender.com/producao", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // Chave para acessar
        },
      ...
      console.error(error);
      setToast({
        msg: error.message || "Erro ao salvar produção.",
        type: "error",
      });
    } finally {
      setLoading(false);
