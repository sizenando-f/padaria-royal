...
import { 
  Calendar, Clock, Thermometer, Trash2, Edit, Wheat, 
  FlaskConical, Download, Upload, Loader2, Search, ArrowLeft,
  Droplets, Star, AlertTriangle, CheckCircle2, MessageSquare
} from 'lucide-react';
...
export default function HistoricoProducao() {
  ...
  const handleImportar = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const res = await fetch('http://localhost:3000/producao/importar', { method: 'POST', body: formData });
      const data = await res.json();
  ...
  const handleDelete = async (id: number) => {
    if(!confirm('Tem certeza que deseja excluir a fornada #' + id + '?')) return;
    try {
      const res = await fetch(`http://localhost:3000/producao/${id}`, { method: 'DELETE' });
  ...
  const renderBadgeNota = (avaliacao: any) => {
    ...
    const map: any = {
        5: { label: 'Excelente', color: 'bg-green-100 text-green-700 border-green-200' },
        4: { label: 'Bom', color: 'bg-blue-100 text-blue-700 border-blue-200' },
        3: { label: 'Regular', color: 'bg-yellow-100 text-yellow-700 border-yellow-200' },
        2: { label: 'Ruim', color: 'bg-orange-100 text-orange-700 border-orange-200' },
        1: { label: 'Péssimo', color: 'bg-red-100 text-red-700 border-red-200' },
    };
    const style = map[avaliacao.nota] || map[3];
    
    return (
        <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border flex items-center gap-1 ${style.color}`}>
           {avaliacao.nota >= 4 && <Star size={10} fill="currentColor"/>} {style.label}
        </span>
    );
  };
    ...
    <div className="min-h-screen bg-gray-50 p-4 pb-24 md:pb-10 flex justify-center">
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="flex flex-col gap-4 mb-6">
            <div className="flex items-center gap-3 pt-4">
                ...
                <div>
                    ...
                    <p className="text-xs text-gray-500">Todas as suas produções registradas</p>
                </div>
            </div>
            ...
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
                ...
                <label className="flex items-center gap-2 bg-white px-4 py-3 rounded-2xl border border-gray-200 text-xs font-bold text-gray-700 shadow-sm active:bg-gray-50 whitespace-nowrap cursor-pointer hover:bg-gray-50 transition-colors">
                    ...
                    <input type="file" accept=".csv" className="hidden" onChange={handleImportar} disabled={importando} />
                </label>
            </div>
        </div>
        ...
        {loading ? (
            ...
            {producoes.map((prod) => (
                <div key={prod.id} className="bg-white p-5 rounded-4xl border border-gray-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <p className="text-xs text-gray-400 flex items-center gap-1">
                                    <Clock size={12}/> {formatHora(prod.horaInicio)} - {formatHora(prod.horaFim)}
                                </p>
                            </div>
                        </div>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-4">
                        ...
                        <div className="bg-gray-50 p-3 rounded-2xl space-y-2">
                            ...
                            <div className="flex items-center justify-between text-xs">
                                <span className="flex items-center gap-1.5 text-gray-600"><Wheat size={14} className="text-yellow-500"/> Farinha</span>
                                <span className="font-bold text-gray-800">{Number(prod.farinhaKg)}kg</span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                                <span className="flex items-center gap-1.5 text-gray-600"><FlaskConical size={14} className="text-purple-500"/> Fermento</span>
                                <span className="font-bold text-gray-800">{Number(prod.fermentoGrama)}g</span>
                            </div>
                            <div className="flex items-center justify-between text-xs">
                                <span className="flex items-center gap-1.5 text-gray-600"><Droplets size={14} className="text-blue-400"/> Emulsif.</span>
                                <span className="font-bold text-gray-800">{Number(prod.emulsificanteMl)}ml</span>
                            </div>
                        </div>
                        ...
                        <div className="bg-blue-50/50 p-3 rounded-2xl space-y-2 border border-blue-50">
                            ...
                            <div className="flex items-center justify-between text-xs">
                                <span className="flex items-center gap-1.5 text-gray-600"><Clock size={14} className="text-orange-400"/> Tempo</span>
                            ...
                            <div className="pt-1 border-t border-blue-100 mt-1">
                                <div className="flex justify-between items-center text-xs mb-1">
                                    <span className="text-gray-500 flex gap-1"><Thermometer size={12}/> Prev.</span>
                                    <span className="font-medium text-gray-800">{prod.tempAmbienteInicial}° ➜ {prod.tempAmbienteFinal}°</span>
                                </div>
                                {prod.avaliacao?.tempAmbienteFinalReal && (
                                    <div className="flex justify-between items-center text-xs bg-blue-100 p-1 rounded-md">
                                        <span className="text-blue-700 font-bold flex gap-1">Real</span>
                                        <span className="font-bold text-blue-900">{prod.avaliacao.tempAmbienteFinalReal}°C</span>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="border-t border-gray-100 pt-3 flex flex-col gap-2">
                            {prod.observacoes && (
                                <p className="text-xs text-gray-500 bg-gray-50 p-2 rounded-lg italic flex gap-2">
                                    <span className="font-bold not-italic text-gray-400 text-[10px] uppercase">Obs:</span> 
                                    {prod.observacoes}
                                </p>
                            )}
